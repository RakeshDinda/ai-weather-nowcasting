"""
Script to generate the complete India-wide locations dataset (1000+ verified locations)
covering ALL 28 States and 8 Union Territories with high density in North, South,
East, West, and especially North-East India.
"""
import os
import csv

# Import the base locations list from the previous generator
from scripts.generate_india_locations import LOCATIONS as BASE_LOCATIONS

ADDITIONAL_LOCATIONS = [
    # --- NORTH-EAST EXPANSION (Extensive Coverage) ---
    # Assam
...
def generate():
    ...
    out_path = os.path.join(out_dir, "india_locations.csv")
    
    seen = set()
    all_rows = []
    
    # Combine base + additional
    combined = list(BASE_LOCATIONS) + ADDITIONAL_LOCATIONS
    
    for city, state, lat, lon in combined:
        key = (city.strip().lower(), state.strip().lower())
        if key in seen:
            continue
        seen.add(key)
        # Coordinate sanity check within India's bounds
        assert 6.0 <= lat <= 38.0, f"Invalid lat: {lat} for {city}"
        assert 68.0 <= lon <= 98.0, f"Invalid lon: {lon} for {city}"
        all_rows.append((city.strip(), state.strip(), round(lat, 4), round(lon, 4)))
    ...
    print(f"Successfully generated {len(all_rows)} locations in {out_path}")
    states = set(r[1] for r in all_rows)
    print(f"Total States & UTs covered: {len(states)}")
    return len(all_rows)
